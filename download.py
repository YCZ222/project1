...
def download_in_parallel(link_list, out_dir, threads=16):
    download_partial = partial(_download_file, out_dir=out_dir)
    with Pool(threads) as p:
        # 用 imap_unordered 替换 map，返回一个迭代器
        result_iter = p.imap_unordered(download_partial, link_list)
        # 用 tqdm.tqdm 包装迭代器，并指定 total 参数为任务总数
        result_iter = tqdm(result_iter, total=len(link_list))
        # 用 list 函数收集结果，并更新进度条
...
if __name__ == "__main__":
    main("./artemis_dataset_release_v0.csv", "./artemis-download", 4)