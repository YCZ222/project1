...
writer = SummaryWriter(log_dir="./Logs/train_style_VIT_0.001")
...
for epoch in tqdm(range(epochs)):
    ...
    with torch.no_grad():
        for images, style_labels, _ in tqdm(test_loader):
            ...
            _, predicted = outputs.max(1)
            total_test += style_labels.size(0)
            correct += predicted.eq(style_labels).sum().item()
    ...
    model_save_path = f"./models/train_style_VIT_0.001/model_epoch_{epoch+1}.pth"
