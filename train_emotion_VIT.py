...
# model
train_model = ViT(class_num=class_num)
...
writer = SummaryWriter(log_dir="./Logs/train_emotion_VIT_0.001")
...
for epoch in tqdm(range(epochs)):
    ...
    for images, _, emotion_labels in tqdm(train_loader):
        ...
        loss = loss_fn(torch.sigmoid(outputs), emotion_labels.float())
        optimizer.zero_grad()
        loss.backward()
        # clip gradient
    ...
    writer.add_scalar("Accuracy/Train", accuracy_train, epoch)  # 记录每个 epoch 的准确率到 TensorBoard
    ...
    with torch.no_grad():
        for images, _, emotion_labels in tqdm(eval_loader):
            ...
            writer.add_scalar("Loss/Eval", loss.item(), total_eval_step)  # 记录每个 step 的损失
            total_eval_step += 1
    ...
    with torch.no_grad():
        for images, _, emotion_labels in tqdm(test_loader):
            ...
            writer.add_scalar("Loss/Test", loss.item(), total_test_step)  # 记录每个 step 的损失
            total_test_step += 1
    ...
    model_save_path = f"./models/train_emotion_VIT_0.001/model_epoch_{epoch+1}.pth"
