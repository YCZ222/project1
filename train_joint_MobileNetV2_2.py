...
from models_final import MobileNetV2_joint
...
# model
train_model = MobileNetV2_joint(class_num1=class_num1, class_num2=class_num2)
train_model.to(device)


# learning
loss_fn_style = torch.nn.CrossEntropyLoss()
loss_fn_emotion = torch.nn.BCELoss()

optimizer = torch.optim.Adam(train_model.parameters(), lr=learning_rate, weight_decay=0) # Adam, weight_decay=weight_decay
writer = SummaryWriter(log_dir="./Logs/train_joint_MobileNetV2_2")
...
for epoch in tqdm(range(epochs)):
    ...
    for images, style_labels, emotion_labels in tqdm(train_loader):
        ...
        loss1 = loss_fn_style(outputs1, style_labels)  # 使用样式标签进行损失计算
        outputs2 = torch.sigmoid(outputs2)
        loss2 = loss_fn_emotion(outputs2, emotion_labels.float())  # 使用情感标签进行损失计算
        loss = loss1 + loss2
        optimizer.zero_grad()
        loss.backward()
        # clip gradient
    ...
    with torch.no_grad():
        for images, style_labels, emotion_labels in tqdm(eval_loader):
            ...
            outputs1, outputs2 = train_model(images)
            loss1 = loss_fn_style(outputs1, style_labels)
            outputs2 = torch.sigmoid(outputs2)
            loss2 = loss_fn_emotion(outputs2, emotion_labels.float())
            loss = loss1 + loss2
    ...
    model_save_path = f"./models/train_joint_MobileNetV2_2/model_epoch_{epoch+1}.pth"
